...
static void hideep_read_info(struct hideep_t *ts)
{
	int ii, jj;
	char temp[128];
...
static int hideep_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	ts->ws = wakeup_source_register(&ts->client->dev, "tsp_wakeup_source");
	...
	if (ts->vr_info->num_tx && ts->vr_info->num_rx)
		ts->proc_data = devm_kzalloc(&client->dev, HIDEEP_PROC_SIZE, GFP_KERNEL);
	if (!ts->proc_data)
		input_err(true, &client->dev, "%s: Failed to allocate proc_data\n", __func__);
