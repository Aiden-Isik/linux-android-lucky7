...
#include <uapi/gpu/arm/v_r44p1/gpu/mali_kbase_gpu_id.h>
#if !MALI_USE_CSF
#include <mali_kbase_hwaccess_jm.h>
#endif
#include <linux/v_r44p1/version_compat_defs.h>
...
void kbase_mmu_disable(struct kbase_context *kctx)
{
	...
	if (flush_err) {
		/* Flush failed to complete, assume GPU has hung and perform a reset to recover */
		dev_err(kbdev->dev,
			"Flush for GPU page table update did not complete on MMU disable for as %d",
			kctx->as_nr);
