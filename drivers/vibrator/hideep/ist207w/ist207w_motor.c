...
static int ist207w_motor_store_fifo_vib(struct device *dev, int file_num)
{
	...
	duration = ktime_to_ms(ktime_get()) - start;
	pr_info("%s file read duration=%lld ms\n", __func__, duration);

	ret = ist207w_motor_fifo_play(pMotorData, (unsigned char *)vib_fw->data, vib_fw->size);
	if (ret < 0) {
...
fw_request_fail:
	...
	mutex_unlock(&pMotorData->lock);
	duration = ktime_to_ms(ktime_get()) - start;
	pr_info("%s total duration=%lld ms -\n", __func__, duration);
...
static int ist207w_motor_platform_init(struct ist207w_motor_data *pMotorData)
{
	...
	for (i = 1; i <= 300; i++) {
		...
		if (ret == 0)
			...
			MOTOR_REG_COMMUNICATION_FLAG, ret);
			if (i == 300)
				goto out;
			else {
